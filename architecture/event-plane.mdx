...
```
Coinbase Prime (WS)         Direct REST (Coinbase Prime SDK)
       │                                  │
       ▼                                  ▼
  trading_server  ──── Redis Pub/Sub ────►  gateway  ────► clients (raw ws)
   (BullMQ jobs,                            (broadcasts to subscribed
    socket.io rooms)                         users via WebSocketsGateway)
       ▲                                  ▲
       └────── internal-only ws (not exposed externally) ──┘
```
