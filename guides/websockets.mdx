---
title: "WebSockets"
description: "Stream order book, order, wallet and deposit updates over one socket."
icon: "bolt"
---

The WebSocket API streams real-time order book, order, wallet and deposit
updates over a single authenticated socket — through the **same gateway** as
the REST API (no separate host or port).
...
```
ws://localhost:5678/api/v1/trading/ws
```

## Authenticating the handshake

The socket uses the **same HMAC scheme as REST** (see
[Authentication](/guides/authentication)). The handshake is an HTTP `GET`
upgrade with an empty body, so sign:
...
part of the signature, so the same signing code works whether you connect to
`:5678` or a production host.
...
- **`user` scope is always you.** The key is taken from your authenticated API
  ...
  ever stream your own orders/deposits. (You can send `"key": "me"` or your own
  id; both resolve to your account.)
...
- The `order` and `deposit` channels are **`user`-scoped only** — there is no
  cross-account "market view." The `wallet` channel is currently **disabled**.
...
`deposit_update`). When you subscribe to an order book you
also receive an immediate snapshot:
...
// Same recipe as the REST guide, fixed to the WS handshake (GET, empty body).
...
const ws = new WebSocket('wss://<your-host>/api/v1/trading/ws', {
